        ...
        if (mRouteLine.getAllStep() != null && mRouteLine.getAllStep().size() > 0) {
            ...
            for (DrivingRouteLine.DrivingStep step : mRouteLine.getAllStep()) {
                ...
                if (step.getEntrance() != null) {
                    overlayOptionses.add((new MarkerOptions()).position(step.getEntrance().getLocation()).anchor(0.5f, 0.5f).zIndex(10)
                            .rotate((360 - step.getDirection())).extraInfo(b).icon(BitmapDescriptorFactory.fromAssetWithDpi("Icon_line_node.png")));
                }
                // 最后路段绘制出口点
